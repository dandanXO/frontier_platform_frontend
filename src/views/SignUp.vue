...
div(class="w-screen h-screen flex justify-center items-center  bg-black-100")
  ...
  div(class="flex gap-x-23 items-start")
    ...
    div(class="w-105")
      div(class="w-full h-127.5 rounded-lg card-shadow px-10 pt-10 pb-9.5 flex flex-col")
        ...
        form
          div(class="grid grid-cols-2 gap-3")
            input-text(v-model:textValue="formData.firstName" :placeholder="$t('a.firstName')+'*'")
            input-text(v-model:textValue="formData.lastName" :placeholder="$t('a.lastName')+'*'")
            input-text(v-model:textValue="formData.email" :placeholder="$t('a.email')+'*'" prependIcon="mail" class="col-span-2" @blur="validateEmailFormat")
            input-password(v-model:textValue="formData.password" :placeholder="$t('a.password')+'*'" class="col-span-2")
      ...
      i18n-t(keypath="a.alreadyHaveAnAccount" tag="p" class="text-black-800 text-body2 font-normal text-center pt-3")
        template(#signIn)
...
div(v-if="isSignUpSuccessfully" class="fixed inset-0 w-full h-full bg-black-100 flex justify-center items-center")
  div(class="w-112 h-92 rounded-md bg-black-0 card-shadow pt-22.5 px-14 pb-15 flex flex-col items-center")
    ...
    btn(size="special" class="w-35 h-10.5" @click="nextAfterSignIn") {{$t('a.next')}}
</template>
