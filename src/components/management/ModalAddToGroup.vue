...
div(class="w-118.5")
  div(class="px-8")
    h6(class="text-h6 text-primary font-bold pb-7.5 text-center") {{$t('b.invitePeopleToJoinGroup', { groupName: group.groupName })}}
    input-text(v-model:textValue="searchInput" size="sm" class="pb-7" prependIcon="search" :placeholder="$t('b.searchMember')")
  overlay-scrollbar-container(class="h-89.5 mx-2 border-t border-b border-primary-thin")
    ...
    p(v-else class="text-body1 text-primary text-center pt-9.5") {{$t('b.noMember')}}
  div(class="py-3 flex justify-center items-center")
    btn(size="sm" :disabled="addedMemberList.length === 0" @click="addMemberToGroup") {{$t('reuse.invite')}}
  div(class="bg-black-100 h-15 pl-8 flex items-center" @click="openModalInviteToOrg")
    ...
    p(class="pl-2.5 text-body1 text-primary") {{$t('b.invitePeopleOutsideOrg')}}
</template>
