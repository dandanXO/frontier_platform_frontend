...
modal-behavior(
      ...
      div(class="flex")
        ...
        div(class="grid gap-y-1")
          div(class="flex items-center")
            svg-icon(iconName="done" size="20" class="text-brand")
  ...
  template(#note)
    div(v-if="isExceedMaterialStorage" class="flex items-center")
      svg-icon(iconName="warning_amber_round" class="text-warn" size="14")
    ...
    div(v-else-if="isExceedU3mStorage" class="flex items-center")
      svg-icon(iconName="error_outline" class="text-black-600" size="14")
      div(class="text-caption leading-1.6 pl-1.5")
