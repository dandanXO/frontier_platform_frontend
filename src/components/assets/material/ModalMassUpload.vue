...
div(class="w-116")
  div(class="px-8 relative")
    ...
    i18n-t(keypath="DD0036" tag="p" class="text-center text-black-900 text-body2 leading-1.6")
      template(#UU0065)
        a(
          v-if="locale === 'en-US'"
          target="_blank"
    ...
    div(class="text-center text-primary text-body2 leading-1.6") {{ $t("DD0037") }}
    input-text-btn(
      class="w-full mt-7.5"
      disabledInput
      :label="$t('DD0038')"
...
export default {
  ...
  setup () {
    ...
    const handleUpload = async () => {
      ...
      const { result } = await store.dispatch('assets/batchUpload', { xlsxFile: binaryFile })
