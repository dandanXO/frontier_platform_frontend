  ...
  template(#content)
    div(class="max-w-286 pl-17 pr-7 mx-auto mt-13")
      div(class="mb-10")
        ...
        div(class="text-primary text-body2 leading-1.6") {{ $t("EE0011") }}
      div(v-if="nonEmptyRowList.length < 1") {{ $t("EE0013") }}
      div(v-else)
        div(class="text-primary font-bold text-body1 mb-5") {{ nonEmptyRowList.length }} {{ $t("EE0012") }}
        div(v-for="row in nonEmptyRowList" :key="row.id" class="flex my-7.5")
          div(class="relative mr-11")
            div(class="w-47.5 h-47.5 rounded border-2 border-solid border-black-400 bg-cover" :class="{ 'border-none': row.faceSide.exist }" :style="getBgImg(row, 'faceSide')")
          div(class="relative mr-11")
            div(class="w-47.5 h-47.5 rounded border-2 border-solid border-black-400 bg-cover" :class="{ 'border-none': row.backSide.exist }" :style="getBgImg(row, 'backSide')")
