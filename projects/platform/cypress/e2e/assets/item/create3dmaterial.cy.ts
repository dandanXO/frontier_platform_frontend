...
context('Verify create 3d material page', () => {
  beforeEach(() => {
    cy.login()
  })
  ...
  it('add one noarmal action', () => {
    ...
    //search the data
    cy.get('[data-cy="assets-item-grid"]').should('be.visible')
    ...
    cy.get('[data-cy="assets-item-grid"]')
      .first()
      .within(() => {
        cy.contains('p', 'Leather print Test').should('be.visible').click()
      })
    ...
    cy.get('[data-cy="create-3d-material"]').click()
    ...
    //     cy.wrap($el).find('canvas').should('exist').should('be.visible')
    //   })
