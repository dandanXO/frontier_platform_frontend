...
export default {
  ...
  actions: {
    ...
    async getEmbedList ({ rootGetters, dispatch }, { targetPage = 1, sharingKey, nodeKey }) {
    ...
    async getEmbedMaterial ({ dispatch }, { sharingKey, nodeKey }) {
      const { data } = await embedApi.getEmbedMaterial({ sharingKey, workspaceNodeId: nodeKey.split('-')[1] })
      dispatch('setEmbedModule', data.result)
    }
  }
}
