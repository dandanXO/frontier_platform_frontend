    ...
    f-popper(class="flex-grow" placement="bottom-start" :offset="[30, 8]")
      template(#trigger)
        div(class="flex items-center cursor-pointer")
          ...
          p(class="text-body1 text-primary font-bold max-w-27.5 truncate leading-1.4 cursor-pointer" data-cy="menu-org_name") {{ org.orgName }}
          f-svg-icon(iconName="keyboard_arrow_down" size="24" class="text-black-600")
        ...
        f-list(class="w-70")
          ...
          f-list-item(class="h-10")
            div(class="pl-4.5 w-full flex justify-between items-center")
              p(class="text-primary text-caption") {{ $t("OO0002") }}: {{ plan.quota.material.used }}/{{ plan.quota.material.isUnlimited ? $t("OO0173") : plan.quota.material.max }}
              //- button(
          ...
          f-list-item(class="h-10")
            div(class="pl-4.5 w-full flex justify-between items-center")
              p(class="text-primary text-caption") {{ $t("OO0003") }}: {{ plan.quota.u3m.used }}/{{ plan.quota.u3m.isUnlimited ? $t("OO0173") : plan.quota.u3m.max }}
              //- button(
              ...
              //- ) {{ $t("UU0074") }}
          div(class="mx-2 my-1 h-px bg-black-400")
          f-list-item(v-permission="FUNC_ID.VISIT_BILLING_PAGE" @click="goToBillings(); collapsePopper()" class="cursor-pointer")
            div(class="w-full flex justify-between items-center")
              ...
              div(v-if="planStatus.INACTIVE" class="flex items-center")
                ...
                f-svg-icon(iconName="arrow_forward" class="text-brand-dark" size="14")
          f-list-item(@click="goToLobby" class="cursor-pointer")
            p(class="pl-4.5") {{ $t("RR0127") }}
    f-popper(placement="bottom-start" @expand="readNotification" @collapse="isExpand = false")
      template(#trigger)
        div(class="relative cursor-pointer")
          f-svg-icon(iconName="notification" class="text-black-700")
      ...
      template(#content)
        div(class="w-75 rounded-md overflow-hidden")
          ...
          f-scrollbar-container(class="max-h-127.5")
            div(class="divide-y divide-black-200 border-t border-b border-black-200")
              div(
                ...
                p(class="text-caption text-primary leading-1.6 pb-1" v-html="notification.content")
          ...
          div(class="bg-black-50 flex justify-center items-center py-2 text-caption text-black-700 leading-1.6 text-center")
