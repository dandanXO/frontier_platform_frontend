    ...
    f-popper(class="flex-grow" placement="bottom-start" :offset="[30, 8]")
      template(#trigger)
        div(class="flex items-center cursor-pointer")
          ...
          p(class="text-body1 text-grey-900 font-bold max-w-27.5 truncate leading-1.4 cursor-pointer" data-cy="menu-org_name") {{ org.orgName }}
    ...
    f-popper(placement="bottom-start" @expand="readNotification" @collapse="isExpand = false")
      template(#trigger)
        div(class="relative cursor-pointer")
          ...
          div(v-if="haveUnreadNotification" class="absolute top-0.5 right-px w-2 h-2 rounded-full border border-grey-0 bg-red-400")
      template(#content)
          ...
          f-scrollbar-container(class="max-h-127.5")
            div(class="divide-y divide-grey-100 border-t border-b border-grey-100")
              div(
                ...
                p(class="text-caption text-grey-900 leading-1.6 pb-1" v-html="notification.content")
                p(class="text-caption text-grey-600") {{ notification.formattedDate }}
                i(v-if="!notification.isRead" class="absolute top-4.5 left-3 w-2 h-2 rounded-full bg-primary-400")
          div(class="bg-grey-50 flex justify-center items-center py-2 text-caption text-grey-600 leading-1.6 text-center")
