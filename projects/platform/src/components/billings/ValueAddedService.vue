      ...
      div(
        ...
        div(class="w-63 flex-none pb-5 px-4.5 flex flex-col h-full justify-between")
          ...
          div(class="flex items-center text-body2")
            template(
              v-if="valueAddedService[recommendService.id].status === VALUE_ADDED_SERVICE_STATUS.ACTIVATE"
            )
              f-svg-icon(
        ...
        div(
          class="w-132 flex-none bg-cover rounded-r"
          :style="{ backgroundImage: `url(${recommendService.bannerImage})` }"
        )
      ...
      div(class="grid")
        template(v-for="service in serviceList")
          div(
            ...
            div(class="flex items-center text-body2")
              template(
                v-if="valueAddedService[service.id].status === VALUE_ADDED_SERVICE_STATUS.ACTIVATE"
              )
                f-svg-icon(
                  iconName="check_circle_outline"
                  size="20"
...
const recommendService = computed(() =>
  serviceList.find((service) => !!service.bannerImage)
)
