...
export default {
  ...
  setup() {
    ...
    const paymentDetail = computed(
      () => store.getters['organization/paymentDetail']
    )
    ...
    const billingInfoItemList = computed(() => {
      ...
      const { recipient, email, countryCode, zipCode, city, address } =
        paymentDetail.value.billingInfo
      const nation = countryList.find(
        (country) => country.countryCode === countryCode
      ).name
      const thirdRow = `${zipCode ? zipCode + ', ' : ''}${
        city ? city + ', ' : ''
      }${nation}`
      return [recipient, email, thirdRow, address]
    })
