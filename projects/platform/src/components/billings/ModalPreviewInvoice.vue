      ...
      div(class="flex items-center mb-4")
        ...
        invoice-pdf-generator
          template(#activator="{ generatePdf }")
            div(
              class="flex items-center text-cyan-400 cursor-pointer"
  ...
  div(class="relative")
    ...
    f-scrollbar-container(class="h-77.5 px-8 my-5")
      div(class="flex flex-col gap-2.5")
        div(
          v-for="item in innerInvoice.serviceItemList"
    ...
    const countryName = computed(
      () =>
        store.getters['code/countryList'].find(
          (country) => country.countryCode === innerInvoice.value.countryCode
        )?.name
    )
    const subtotalPrice = computed(() =>
      innerInvoice.value.serviceItemList.reduce(
        (prev, current) => prev + current.price,
        0
      )
    )
    const addressDetail = computed(() =>
      [innerInvoice.value.zipCode, innerInvoice.value.city, countryName.value]
        .filter((v) => !!v)
        .join(', ')
    )
        ...
        properties: {
          ...
          actionHandler: async (billingInfo) => {
            ...
            const { invoiceInfo } = await store.dispatch(
              'organization/updateInvoiceBillingInfo',
              { ...billingInfo, invoiceId: props.invoiceInfo.invoiceId }
            )
