            ...
            div(v-if="currExecOptionIndex === index" class="w-149 h-194 bg-grey-0 pt-6")
              ...
              div(class="relative")
                ...
                div(class="h-77.5 px-8 my-5")
                  div(class="flex flex-col gap-2.5")
                    div(v-for="item in serviceItemList" class="flex text-caption text-grey-900")
                      div(style="width: 50%")
              ...
              div(class="px-8 mt-3 flex justify-between")
                div
                  ...
                  div(class="grid grid-cols-2 grid-rows-3 w-fit gap-y-2 gap-x-4.5 leading-none text-grey-600 text-caption")
                    div {{ $t("OO0095") }}
                ...
                div(class="relative h-full")
                  ...
                  div(v-if="index + 1 === serviceItemListByPage.length" class="absolute -right-8 top-14.5 w-60 py-2 px-10 bg-primary-0 text-grey-900 text-body2")
...
export default {
  ...
  setup () {
    ...
    const countryName = computed(() => store.getters['code/countryList'].find(country => country.countryCode === invoiceInfo.value.countryCode)?.name)
    const addressDetail = computed(() => [invoiceInfo.value.zipCode, invoiceInfo.value.city, countryName.value].filter(v => !!v).join(', '))

    const generatePdf = async (invoiceId) => {
      store.dispatch('helper/pushModalLoading')
      serviceItemListByPage.length = 0
      currExecOptionIndex.value = 0
      const result = await store.dispatch('organization/getInvoiceDetail', { invoiceId })
      invoiceInfo.value = result.invoiceInfo

      isShown.value = true

      const itemsPerPage = 8
      const totalPage = Math.ceil(invoiceInfo.value.serviceItemList.length / itemsPerPage)

      for (let i = 0; i < totalPage; i++) {
        const part = invoiceInfo.value.serviceItemList.slice(i * itemsPerPage, itemsPerPage * (i + 1))
    ...
    const calcSubtotal = (items) => items.reduce((prev, current) => prev + current.price, 0)
