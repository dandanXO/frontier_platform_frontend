  ...
  div(class="w-212.5 mx-auto pt-16.5")
    div(class="mb-16")
      ...
      div(class="flex gap-2")
        f-tag(v-for="(tag, index) in tagList" :active="indexOfActiveTag === index" @click="indexOfActiveTag = index") {{ tag }}
    div(v-if="tagList[indexOfActiveTag] === MADE2FLOW_TAG_LIST.INTRODUCTION.text")
      div
      ...
      div(class="mt-10") 
        ...
        article(class="text-body2 leading-1.6 text-grey-900 pb-22.5")
          ...
          br
          p {{ $t("M2F048") }}
    div(v-else-if="tagList[indexOfActiveTag] === MADE2FLOW_TAG_LIST.METHODOLOGY.text" class="pb-19")
    ...
    div(v-else-if="tagList[indexOfActiveTag] === MADE2FLOW_TAG_LIST.FAQ.text" class="pb-19")
    ...
    div(v-else-if="tagList[indexOfActiveTag] === MADE2FLOW_TAG_LIST.PLAN_AND_PRICE.text")
    ...
    value-added-service-made2flow-appointment(v-else-if="tagList[indexOfActiveTag] === MADE2FLOW_TAG_LIST.APPOINTMENT.text")
</template>
...
const tagList = computed(() => tagListKeys.map(key => MADE2FLOW_TAG_LIST.value[key].text))
...
const goToAppointment = () => {
  indexOfActiveTag.value = tagListKeys.findIndex(key => MADE2FLOW_TAG_LIST.value[key].id === MADE2FLOW_TAG_LIST.value.APPOINTMENT.id)
}
...
watch(
  ...
  () => {
    if (route.query.tagId) {
      indexOfActiveTag.value = tagListKeys.findIndex(key => MADE2FLOW_TAG_LIST.value[key].id === Number(route.query.tagId))
    }
  },
  {
    immediate: true
