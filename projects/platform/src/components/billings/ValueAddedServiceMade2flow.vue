  ...
  div(class="w-212.5 mx-auto pt-16.5")
    div(class="mb-16")
      ...
      div(class="flex gap-2")
        f-tag(v-for="(tag, index) in tagList" :active="activeTag === index" @click="activeTag = index") {{ tag }}
    div(v-if="tagList[activeTag] === MADE2FLOW_TAG_LIST.INTRODUCTION.text")
      div
        ...
        div(class="grid gap-x-3 grid-cols-3 w-fit mx-auto mt-4")
          div(v-for="(item, index) in 3" class="w-3 h-3 rounded-full" :class="[currentSlide === index ? 'bg-grey-900' : 'bg-grey-150']")
      div(class="mt-10") 
        ...
        article(class="text-body2 leading-1.6 text-grey-900 pb-22.5")
          ...
          p {{ $t("M2F046") }}
          br
          i18n-t(keypath="M2F047" tag="p")
            template(#newline)
              br
          br
          p {{ $t("M2F048") }}
    div(v-else-if="tagList[activeTag] === MADE2FLOW_TAG_LIST.METHODOLOGY.text" class="pb-19")
    ...
    div(v-else-if="tagList[activeTag] === MADE2FLOW_TAG_LIST.FAQ.text" class="pb-19")
    ...
    div(v-else-if="tagList[activeTag] === MADE2FLOW_TAG_LIST.PLAN_AND_PRICE.text")
    ...
    value-added-service-made2flow-appointment(v-else-if="tagList[activeTag] === MADE2FLOW_TAG_LIST.APPOINTMENT.text")
</template>
...
import { MADE2FLOW_TAG_LIST, VALUE_ADDED_SERVICE_ID } from '@/utils/constants'
...
const made2flow = valueAddedServiceList.find(service => service.id === VALUE_ADDED_SERVICE_ID.MADE2FLOW)
const faqList = ref(made2flow.detail.faqList)
faqList.value = made2flow.detail.faqList.map(faq => {
  return { ...faq, isExpand: false }
})

const tagListKeys = Object.keys(MADE2FLOW_TAG_LIST)
const tagList = computed(() => tagListKeys.map(key => MADE2FLOW_TAG_LIST[key].text))
const breadcrumbList = computed(() => {
  return [
    ...
    {
      name: made2flow.projectName,
      path: parsePath(`${prefixPath.value}/billings/value-added-service?service=${Number(route.query.service)}`)
    }
  ]
})

const goToAppointment = () => {
  activeTag.value = tagListKeys.findIndex(key => MADE2FLOW_TAG_LIST[key].id === MADE2FLOW_TAG_LIST.APPOINTMENT.id)
}
...
watch(
  ...
  () => {
    if (route.query.tagId) {
      activeTag.value = tagListKeys.findIndex(key => MADE2FLOW_TAG_LIST[key].id === Number(route.query.tagId))
    }
  },
  {
    immediate: true
