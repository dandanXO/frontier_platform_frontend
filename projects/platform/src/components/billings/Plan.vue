...
div(class="w-195")
  p(class="text-body1 text-grey-900 mt-18.5 mb-5 flex") {{ $t('OO0008') }}
    span(v-if="!planType.ENT && planStatus.BUFFER" class="flex items-center pl-5")
      ...
      i18n-t(keypath="WW0088" tag="span" class="text-caption text-red-400 pl-1" scope="global")
        template(#UU0082)
  ...
  div(class="grid grid-cols-3 grid-rows-2 gap-3 h-78")
    div(class="col-span-3 border border-grey-200 rounded px-7.5 flex justify-between items-center")
      div
      ...
      f-button(v-if="!planType.ENT" size="lg" :disabled="!planStatus.ACTIVE" @click="openModalChoosePlan") {{ $t('UU0075') }}
    div(class="border border-grey-200 rounded pt-8 pr-3 pb-6 pl-7 flex justify-between")
      div
        ...
        p(class="text-body1 font-bold leading-1.6" :class="[isMaterialFull ? 'text-red-400' : 'text-primary-500']") {{ materialQuota.used }}/{{ materialQuota.isUnlimited ? $t("OO0173") : materialQuota.max }} 
    ...
    div(class="border border-grey-200 rounded pt-8 pr-3 pb-6 pl-7 flex justify-between")
      div
        ...
        p(class="text-caption text-grey-600 leading-1.6 mb-5") {{ $t('OO0137') }}
        p(class="text-body1 font-bold leading-1.6" :class="[isU3mFull ? 'text-red-400' : 'text-primary-500']") {{ u3mQuota.used }}/{{ u3mQuota.isUnlimited ? $t("OO0173") : u3mQuota.max }}
          span(class="text-caption font-normal pl-1") {{ $t('OO0035') }}
      f-circle-progress-bar(
        class="self-end"
        :size="60"
  ...
  template(v-if="!planType.ENT")
    p(v-if="planStatus.ACTIVE || planStatus.BUFFER" class="text-body2 text-cyan-400 text-right pt-3 cursor-pointer" @click="deactivateOrg") {{ $t('OO0007') }}
    div(v-else-if="planStatus.INACTIVE" class="w-full h-24 bg-grey-100 flex justify-between items-center pl-7.5 pr-10 rounded mt-6")
      div
