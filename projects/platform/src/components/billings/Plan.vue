...
div(class="w-195")
  p(class="text-body1 text-primary mt-18.5 mb-5 flex") {{ $t('OO0008') }}
    span(v-if="!planType.ENT && planStatus.BUFFER" class="flex items-center pl-5")
      f-svg-icon(iconName="warning_amber_round" size="20" class="text-warn")
  ...
  div(class="grid grid-cols-3 grid-rows-2 gap-3 h-78")
    ...
    div(class="border border-black-400 rounded pt-8 pr-3 pb-6 pl-7 flex justify-between")
      div
        ...
        p(class="text-body1 font-bold leading-1.6" :class="[isMaterialFull ? 'text-warn' : 'text-brand-dark']") {{ materialQuota.used }}/{{ materialQuota.isUnlimited ? $t("OO0173") : materialQuota.max }} 
    ...
    div(class="border border-black-400 rounded pt-8 pr-3 pb-6 pl-7 flex justify-between")
      div
        ...
        p(class="text-caption text-black-700 leading-1.6 mb-5") {{ $t('OO0137') }}
        p(class="text-body1 font-bold leading-1.6" :class="[isU3mFull ? 'text-warn' : 'text-brand-dark']") {{ u3mQuota.used }}/{{ u3mQuota.isUnlimited ? $t("OO0173") : u3mQuota.max }}
          span(class="text-caption font-normal pl-1") {{ $t('OO0035') }}
      f-circle-progress-bar(
        class="self-end"
        :size="60"
