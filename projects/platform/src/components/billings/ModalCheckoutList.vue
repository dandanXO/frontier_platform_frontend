  ...
  template(#note)
    div(class="flex items-center")
      ...
      i18n-t(
        keypath="OO0167"
        tag="p"
        class="text-start text-grey-600 text-caption leading-1.6"
        scope="global"
      )
        template(#newline)
          br
        template(#OO0116)
          span(
            ...
            @click="openModalTermsOfSubscription"
          ) {{ $t('OO0116') }}
  div(class="w-117")
    i18n-t(
      v-if="isChargeNow"
      keypath="OO0166"
      tag="p"
      class="text-center text-grey-900 text-caption leading-1.6 pb-3.5"
      scope="global"
    )
      template(#newline)
        br
...
const cardInfo = computed(
  () => store.getters['organization/paymentDetail'].cardInfo
)
