...
modal-behavior(
  ...
  div(class="w-80 flex flex-col items-center")
    div(
      class="h-10 w-full text-caption text-grey-600 font-bold bg-grey-50 mb-2 flex items-center justify-center"
    ) {{ email }}
    div(class="text-caption text-grey-900 mb-4") {{ $t('AA0053') }}
    form(class="w-full pb-4")
      f-input-password(
        v-model:textValue="password"
        :placeholder="$t('AA0055')"
      )
      password-validator(
...
const errorMsg = computed(() => {
  ...
  return password.value !== confirmPassword.value ? t('WW0070') : ''
})
...
const changeHandler = async () => {
  switch (props.mode) {
    case MODE.FORGOT:
      ...
      break
    case MODE.OLD_RESET:
      await store.dispatch('user/oldUserResetPassword', {
        password: password.value,
