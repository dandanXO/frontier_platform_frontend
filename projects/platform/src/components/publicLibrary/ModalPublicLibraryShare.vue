...
modal-behavior(
  ...
  div(class="w-103.5")
    div(class="flex items-end justify-between pb-7.5 gap-7")
      ...
      f-button(size="sm" class="flex-shrink-0" @click="openModalPublicLibraryShareAssigned") {{ $t("UU0067") }}
  div(class="grid gap-10 grid-cols-4 bg-grey-50 py-5 px-10 justify-items-center text-grey-900")
    ...
    div(:class="[isCanShared ? 'cursor-pointer' : '']" @click="generateCopyLink")
      div(class="w-14 h-14 flex justify-center items-center bg-grey-100 rounded-full")
        f-svg-icon(iconName="link_1" size="30" class="text-grey-900 transform -rotate-45" :class="{ 'text-grey-200': !isCanShared }")
      p(class="text-caption text-center pt-3" :class="{ 'text-grey-200': !isCanShared }") {{ $t("RR0154") }}
</template>
