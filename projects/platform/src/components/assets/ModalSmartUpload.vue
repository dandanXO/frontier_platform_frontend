    ...
    template(v-if="readyToUploadFile.length === 0")
      ...
      div(
        ...
        div(class="w-44 flex flex-col items-center")
          f-svg-icon(iconName="cloud_upload" size="68" class="text-brand")
        ...
        div(class="grid divide-y divide-black-200")
          div(v-for="image in readyToUploadFile" class="py-1 h-11 flex items-center" data-cy="modal-smart-upload_item")
            div(class="group py-1 h-9 rounded flex items-center px-5 hover:bg-black-200")
              ...
              div(class="w-5 flex-shrink-0")
                f-svg-icon(v-if="image.processing !== 1" iconName="clear" size="20" class="cursor-pointer text-black-600 invisible group-hover:visible" data-cy="modal-smart-upload_remove" @click="image.isRemoved = true")
                f-svg-icon(v-else iconName="done" size="20" class="text-brand" data-cy="modal-smart-upload_done")
</template>
