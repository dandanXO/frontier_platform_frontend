  ...
  () => {
    if (faceSideConfig?.config) {
      const baseWidth = Math.min(
        faceSideConfig.config.image.width,
        faceSideConfig.config.image.height
      )
      faceSideConfig.config.scaleRatio =
        (baseWidth * (pxPerCm / faceSideImg.dpi)) / scaleSize.value
    }
    if (backSideConfig?.config) {
      const baseWidth = Math.min(
        backSideConfig.config.image.width,
        backSideConfig.config.image.height
      )
      backSideConfig.config.scaleRatio =
        (baseWidth * (pxPerCm / backSideImg.dpi)) / scaleSize.value
    }
  }
)
...
const confirm = async () => {
  ...
  const faceSideCropImg = faceSideUrl
    ? await refFaceSide.value?.cropImage()
    : null
  const backSideCropImg = backSideUrl
    ? await refBackSide.value?.cropImage()
    : null
  ...
  if (faceSideUrl) {
    ...
    Object.assign(faceSideConfig, {
      ...
      config: {
        ...
        scaleRatio:
          (faceSideCropper.config.image.width * (pxPerCm / faceSideImg.dpi)) /
          scaleSize.value,
      },
    })
  ...
  if (backSideUrl) {
    ...
    Object.assign(backSideConfig, {
      ...
      config: {
        ...
        scaleRatio:
          (backSideCropper.config.image.width * (pxPerCm / backSideImg.dpi)) /
          scaleSize.value,
      },
    })
