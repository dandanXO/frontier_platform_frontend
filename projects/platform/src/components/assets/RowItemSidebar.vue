...
div(class="text-grey-600 flex flex-col gap-3.5")
  f-tooltip(v-for="item in [editMaterial, printCard, downloadU3M]" class="cursor-pointer" boundaryReference="search-table-header")
    template(#trigger)
      div(v-if="item.id === 'downloadU3M' && material.u3m.status !== U3M_STATUS.COMPLETED" class="w-7.5 h-7.5 flex justify-center items-center text-grey-200")
        f-svg-icon(:iconName="item.icon" size="24")
      div(v-else class="w-7.5 h-7.5 hover:bg-primary-400/10 hover:text-primary-400 flex justify-center items-center rounded-full" @click="item.func && item.func(material)")
        f-svg-icon(:iconName="item.icon" size="24")
    template(#content)
      p {{ item.name }}
  f-popper(class="cursor-pointer" placement="left-start")
    template(#trigger)
      div(class="w-7.5 h-7.5 hover:bg-primary-400/10 hover:text-primary-400 flex justify-center items-center rounded-full")
