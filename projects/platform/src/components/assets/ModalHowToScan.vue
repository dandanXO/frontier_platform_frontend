  ...
  template(#note v-if="materialList.length > 0")
    div(class="flex items-center text-grey-600 w-170")
      ...
      i18n-t(keypath="DD0032" tag="div" class="text-caption leading-1.6" scope="global")
        template(#RR0062)
          div(class="inline-flex items-center text-cyan-400 cursor-pointer" @click="printA4Card(materialList)") {{ $t("RR0062") }}
            f-svg-icon(iconName="open_in_new" size="15")
        template(#RR0061)
          div(class="inline-flex items-center text-cyan-400 cursor-pointer" @click="printGeneralLabel(materialList)") {{ $t("RR0061") }}
            f-svg-icon(iconName="open_in_new" size="15")
  div(class="w-230")
