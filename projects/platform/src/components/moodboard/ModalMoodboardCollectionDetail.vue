...
modal-behavior(
  ...
  div(class="w-151.5 h-52.5 flex justify-between")
    div(class="w-94.5 flex flex-col")
      ...
      f-scrollbar-container(v-if="description" class="flex-grow -ml-6.5 px-6.5 break-all text-body2 text-grey-900 leading-1.6")
        pre(class="whitespace-pre-wrap" :style="{ 'word-break': 'break-word', 'font-family': 'unset' }") {{ description }}
    ...
    div(class="w-44 flex flex-col")
      ...
      a(v-if="trendBoardCoverImg" :href="trendBoardUrl" target="_blank"  class="flex-grow flex flex-col bg-grey-200 rounded overflow-hidden")
        ...
        div(class="flex-grow bg-grey-50 flex items-center justify-center")
          p(class="text-caption text-grey-900 w-37.5 line-clamp-1 !break-all") {{ trendBoardFileName }}
