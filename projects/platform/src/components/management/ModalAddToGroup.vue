...
modal-behavior(
  ...
  :primaryBtnText="`${$t('UU0014')}(${addedMemberList.length})`"
  ...
  div(class="w-94")
    ...
    f-input-text(
      ...
      class="mb-1.5"
      prependIcon="search"
      :placeholder="$t('BB0012')"
    )
    f-scrollbar-container(class="h-75 mt-2 -mx-5")
      div(
        v-if="memberList.length > 0"
        class="grid gap-y-1 mx-5"
        @mouseleave="indexOfOnHover = -1"
      )
        label(
          v-for="(member, index) in memberList"
          class="pl-2 pr-4 h-12 flex items-center justify-between hover:bg-grey-100"
          :for="member.orgUserId"
          @mouseenter="indexOfOnHover = index"
        )
          div(class="flex items-center")
            div(
              class="w-9 h-9 rounded-full bg-cover bg-center"
              :class="{ 'opacity-30': member.isPending || member.joined }"
            ...
            template(v-else-if="member.joined")
              span(
                class="flex-grow pl-4 text-body2 text-grey-250 truncate w-64 h-6 flex items-center"
              ) {{ member.displayName }}
              span(class="text-body2 text-grey-250") ({{ $t('BB0097') }})
            template(v-else)
              p(
                class="flex-grow pl-4 text-body2 text-grey-900 truncate w-64 h-6 flex items-center"
              ) {{ member.displayName }}
  ...
  template(#note)
    div(
      class="flex items-center cursor-pointer text-cyan-400"
      @click="openModalInviteToOrg"
    )
...
const memberList = computed(() => {
  return orgMemberList.value
    .filter(
      (orgMember) =>
        orgMember.displayName?.includes(searchInput.value) ||
        orgMember.email.includes(searchInput.value)
    )
    .map((orgMember) => {
      const joined =
        [ROLE_ID.OWNER, ROLE_ID.ADMIN].includes(orgMember.orgRoleId) ||
...
const addMemberToGroup = async () => {
  ...
  await store.dispatch('group/addMemberToGroup', {
    orgUserIdList: addedMemberList.value,
  })
