...
div(class="flex flex-col")
  ...
  div(class="mt-2 mb-10 relative")
    div(class="px-10 h-10 grid grid-cols-12 gap-x-6 items-center bg-grey-100 text-body2 text-grey-600")
      ...
      template(v-else-if="planType.STANDARD")
        ...
        div(class="col-span-6 bg-contain bg-no-repeat flex items-center bg-grey-0" :style="{ backgroundImage: `url(${maskHalf})` }")
          div(class="flex flex-col items-center justify-center w-full")
            ...
            template(v-else)
              f-svg-icon(iconName="subscribe" size="30" class="mb-3.5 text-grey-900")
              i18n-t(keypath="VV0071" scope="global" tag="p" class="text-center text-body2 leading-1.6 text-grey-900")
                template(#newline)
                  br
      template(v-else)
        ...
        div(class="col-span-9 bg-contain bg-no-repeat flex items-center bg-grey-0" :style="{ backgroundImage: `url(${maskFull})` }")
          div(class="flex flex-col items-center justify-center w-full")
            template(v-if="isInnerApp")
              ...
              i18n-t(keypath="VV0004" scope="global" tag="p" class="mb-5 text-caption text-grey-600 leading-1.6")
    ...
    div(class="pt-2 grid gap-y-1 relative")
      div(
        ...
        div(class="col-span-3 h-9.5 flex items-center gap-x-3" :class="[made2flowSubscribed ? 'text-grey-900' : 'text-grey-200']")
        ...
        div(class="col-span-7")
          ...
          div(v-if="isShowGraph" class="pt-2")
            div(class="relative")
              div(class="grid grid-cols-5 h-10 divide-x divide-grey-100 bg-grey-50 border border-grey-100 rounded mb-3.5")
                div(v-for="i in 5" class="relative")
                  span(v-if="i === 1" class="absolute -bottom-0.5 translate-y-full left-0 text-caption text-grey-200") 0
                  span(class="absolute -bottom-0.5 translate-y-full right-0 text-caption text-grey-200") {{ i * 10 }}
              div(class="border-0 absolute z-1 left-0 top-3 bg-primary-400 h-1" :style="{ 'width': 100 * (property.personalized / 50) + '%' }")
              div(class="border-0 absolute z-2 right-1 top-1 text-primary-500 text-caption") {{ property.personalized }}
  ...
  div(v-if="made2flowSubscribed" class="flex items-center gap-x-4")
    ...
    div(v-if="isInnerApp" class="flex items-center gap-x-1.5 text-grey-600 cursor-pointer" @click="goToAppointment")
...
const goToAppointment = () => {
  router.push({ name: 'Billings', params: { tab: 'value-added-service' }, query: { service: VALUE_ADDED_SERVICE_ID.MADE2FLOW, tagId: MADE2FLOW_TAG_LIST.value.APPOINTMENT.id } })
}
</script>
