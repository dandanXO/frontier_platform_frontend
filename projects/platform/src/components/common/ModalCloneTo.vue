...
const selectedOrgId = ref(
  props.crossOrg
    ...
    : store.getters['organization/orgId']
)
...
watch(
  ...
  () => {
    ...
    isExceedMaterialStorage.value =
      selectedLocationList.value.length * estimatedQuota.value.material >
      materialMax - materialUsed

    const { u3mUsed, u3mMax } = remainingQuota.value.u3m
    isExceedU3mStorage.value =
      selectedLocationList.value.length * estimatedQuota.value.u3m >
      u3mMax - u3mUsed
...
const submit = async () => {
  const targetLocationList = selectedLocationList.value.map(
