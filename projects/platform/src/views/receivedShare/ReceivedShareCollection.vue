  ...
  search-table(
    ...
    template(#header-left="{ goTo }")
      div(class="flex items-start")
        ...
        f-tooltip
          template(#trigger)
            f-svg-icon(iconName="clone" class="text-grey-600 hover:text-primary-400 cursor-pointer" size="24" @click="receivedShareCloneByNodeKey(currentNodeKey)")
          template(#content)
            p {{ $t("RR0056") }}
    template(#sub-header)
          ...
          div(class="w-full h-81 border border-grey-150 rounded-b !border-t-0 py-6 pl-7 pr-11 flex justify-between")
            ...
            div(class="relative w-97.5 h-69 rounded bg-grey-200 flex items-center justify-center flex-shrink-0")
              div(v-if="collection.trendBoardCoverImg" class="rounded w-full h-full px-7.5 py-6 bg-grey-100")
                div(class="w-full h-full bg-contain bg-no-repeat bg-center rounded bg-grey-0" :style="{ backgroundImage: `url(${collection.trendBoardCoverImg})`}")
