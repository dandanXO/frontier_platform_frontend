...
fullscreen-header
  ...
  template(#right)
    div(class="relative cursor-pointer mr-4" @click="openModalShareMessage")
      ...
      div(v-if="hasMsg" class="absolute -top-px -right-px w-2 h-2 rounded-full border border-grey-0 bg-red-400")
  ...
  template(#content)
    div(class="w-227 mx-auto mt-11")
      ...
      div(class="mb-6 flex")
        div(class="relative w-97.5 h-69 rounded bg-grey-200 flex items-center justify-center flex-shrink-0 mr-9")
    ...
    div(class="fixed z-footer bottom-0 w-full h-13 bg-grey-50 px-36 flex items-center justify-end card-shadow")
