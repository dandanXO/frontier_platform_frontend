  ...
  main(class="flex-grow relative")
    ...
    div(
      ...
      div(class="w-192.5 h-119.5 bg-grey-0 rounded relative flex overflow-hidden" @click.stop)
        f-svg-icon(iconName="clear" class="absolute top-6 right-6 text-grey-600 cursor-pointer" @click="isShowTitasAnnouncement = false")
        ...
        div(class="flex-grow pt-15 pl-8.5 pr-8")
          ...
          div(class="pt-7.5 pb-3 grid grid-flow-col gap-x-3 justify-start")
            div(v-for="org in titasInfo.orgList" class="rounded-full card-shadow w-10 h-10 overflow-hidden border border-grey-0")
              img(:src="org.logo" class="w-full h-full")
          p(class="w-84.5 text-body2 text-grey-400 leading-1.6 pb-5") {{$t('II0017')}}
          f-button(size="md" @click="goToTitasShowroom(); isShowTitasAnnouncement = false") {{$t('UU0118')}}
          div(class="my-6 w-full h-px bg-grey-100")
          p(class="text-body2 leading-1.6")
            span(class="text-grey-400") {{$t('II0018')}}
            a(href="https://event.frontier.cool/titas2022" target="_blank" class="text-cyan-400") &nbsp;&nbsp;{{$t('II0019')}}
</template>
...
const NotifyBarBuffer = defineAsyncComponent(() => import('@/components/billings/NotifyBarBuffer.vue'))
