      ...
      div(class="ml-5 mt-15.5")
        ...
        div(class="flex justify-between items-end")
          div(class="text-grey-900 text-body1 leading-1.6")
            p {{ $t("DD0079") }}
            p {{ $t("DD0080") }}
          div(class="flex items-center text-grey-600 cursor-pointer hover:text-grey-900" @click="openModalBackSideQrcode")
        ...
        div(class="grid grid-cols-3 gap-7.5")
          div
            div(class="border border-grey-200 card-shadow rounded-md")
              ...
              div(class="flex min-h-20 bg-grey-100 rounded-b-md p-4")
                div(class="mr-2 flex-shrink-0 bg-grey-0 text-grey-200 rounded-full w-6 h-6 flex items-center justify-center") 1
          ...
          div
            div(class="border border-grey-200 card-shadow rounded-md")
              ...
              div(class="flex min-h-20 bg-grey-100 rounded-b-md p-4")
                div(class="mr-2 flex-shrink-0 bg-grey-0 text-grey-200 rounded-full w-6 h-6 flex items-center justify-center") 2
          ...
          div
            div(class="border border-grey-200 card-shadow rounded-md")
              ...
              div(class="flex min-h-20 bg-grey-100 rounded-b-md p-4")
                div(class="mr-2 flex-shrink-0 bg-grey-0 text-grey-200 rounded-full w-6 h-6 flex items-center justify-center") 3
                div(class="text-grey-900 text-body1 font-bold leading-1.6")
                  div {{ $t("DD0084") }}
                  i18n-t(keypath="DD0085" tag="div" scope="global")
                    template(#RR0008)
                      span(class="text-cyan-400 underline cursor-pointer" @click="goToAssets") {{ $t("RR0008") }}
        div(class="pt-26 pb-18")
          div(class="text-h6 font-bold text-grey-600 pb-5") {{ $t("DD0087") }}
          div(class="grid gap-3")
            div(v-for="option in alternativeUploadOptions" class="flex py-6 px-7.5 cursor-pointer border border-grey-200 rounded" :data-cy="option.testId" @click="option.action")
