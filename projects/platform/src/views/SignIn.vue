  ...
  div(class="h-125 flex items-center")
    ...
    div(class="w-105")
      div(class="w-full rounded-lg card-shadow px-10 py-11 flex flex-col")
        p(class="text-grey-900 text-h6 font-bold text-center pb-5.5 border-b border-grey-200") {{ $t("AA0001") }}
        ...
        span(class="self-end text-caption text-grey-600 mb-4 cursor-pointer" @click="openModalForgotPasswordEmail") {{ $t("UU0043") }}
        ...
        div(class="flex-grow text-caption mt-1.5 h-5")
          p(v-if="errorMsgSignIn !== ''" class="text-red-400 text-center" data-cy="errorMsg") {{ errorMsgSignIn }}
      ...
      i18n-t(keypath="UU0045" tag="p" class="text-grey-600 text-body2 font-normal text-center pt-3" scope="global")
        template(#signUp)
          router-link(class="text-grey-900 font-bold ml-3" :to="{ path: '/sign-up', query: $route.query }" data-cy="sign-up-page") {{ $t("AA0016") }}
</template>
