...
export default function useEnumText() {
  const { t } = useI18n()

  const LengthUnitText: { [key in LengthUnit]: string } = {
    [LengthUnit.CM]: t('EE0099'),
    [LengthUnit.INCH]: t('RR0020'),
  }

  const WeightUnitText: { [key in WeightUnit]: string } = {
  ...
  const MaterialTypeText: { [key in MaterialType]: string } = {
  ...
  const SortByText: { [key in PaginationReqSortEnum]: string } = {
  ...
  const AssetsFilterStatusText: { [key in AssetsFilterStatusEnum]: string } = {
