...
export default function useMoodboardDetail({
  ...
  const switchTab = (tab) => {
    ...
    router.push({
      ...
      query: {
        ...
        nodeId: moodboardOfferNodeCollection.value.locationList[0].nodeId,
      },
    })
  }
  ...
  const goTo = (nodeId) => {
    ...
    router.push({
      name: route.name,
      query: { tab: currentTab.value, offerId: currentOfferId.value, nodeId },
    })
  }
  ...
  watch(
    ...
    () => {
      ...
      if (
        route.name === 'OrgMoodboardDetail' ||
        route.name === 'GroupMoodboardDetail'
      ) {
        search()
      }
    }
