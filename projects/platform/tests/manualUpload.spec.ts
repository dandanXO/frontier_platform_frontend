...
const addChips = async (page: Page, target: string, total: number) => {
  ...
  for (let i = 0; i < numOfTypeManually; i++) {
    await page.locator(`[data-cy="${target}"]`).locator('[data-cy="input-chips"]').click({ force: true })
    await page.locator(`[data-cy="${target}"]`).locator('input').fill(`${randomString(randomNumber(0, 10), characters)}{enter}`, { force: true })
    await page.mouse.click(0, 0)
  }
  for (let i = 0; i < numOfSelectFromList; i++) {
    await page.locator(`[data-cy="${target}"]`).locator('[data-cy="input-chips"]').click({ force: true })
    await page.locator('[data-cy="list-item"]').nth(randomNumber(0, 10)).click({ force: true })
    await page.mouse.click(0, 0)
  }
}

const addContent = async (page: Page, index = 0, percentage = 100) => {
  await page.locator('[data-cy="content"]').locator('[data-cy="input-select"]').nth(index).click({ force: true })
  await page.locator('[data-cy="list-item"]').nth(index).click({ force: true })
  await page.locator('[data-cy="percentage"]').locator('input').nth(index).fill(percentage.toString(), { force: true })
}
...
test.describe('Create Material', () => {
  ...
  test.describe('Case 2: Success', () => {
    ...
    test('Case 2-1: Filled complete, direct to assets page', async ({ page }) => {
      for (const key in material) {
        await page.mouse.click(0, 0)
        if (Array.isArray(material[key])) {
          await addChips(page, key, randomNumber(0, 5))
        } else {
          await page.locator(`[data-cy="${key}"] input`).fill(material[key].toString(), { force: true })
        }
      }
    ...
    test('Case 2-2: After revising, submit and direct to assets page', async ({ page }) => {
      for (const key in material) {
        if (Array.isArray(material[key])) {
          await addChips(page, key, randomNumber(0, 5))
        } else {
  ...
  test.describe('Case 3: Failed', () => {
    ...
    Object.keys(material).forEach(name => {
      if (required.includes(name)) {
        test(`Case 3-${Object.keys(material).indexOf(name) + 1}: ${name} never filled, show error message`, async ({ page }) => {
          for (const key in material) {
            if (name !== key) {
              if (Array.isArray(material[key])) {
                await addChips(page, key, randomNumber(0, 5))
    ...
    Object.keys(material).forEach(name => {
      if (required.includes(name)) {
        test(`Case 3-${Object.keys(material).indexOf(name) + 4}: ${name} value is invalid, show error message`, async ({ page }) => {
          for (const key in material) {
            if (name !== key) {
              if (Array.isArray(material[key])) {
                await addChips(page, key, randomNumber(0, 5))
          ...
          await page.locator(`[data-cy="${name}"] input`).fill("", { force: true })
          await addContent(page, 0, 100)
        })
      }
    })

    test('Case 3-7: ContentList is Empty, show error message', async ({ page }) => {
      for (const key in material) {
        if (Array.isArray(material[key])) {
          await addChips(page, key, randomNumber(0, 5))
        } else {
    ...
    test('Case 3-8: ContentList value is invalid, show error message', async ({ page }) => {
      for (const key in material) {
        if (Array.isArray(material[key])) {
          await addChips(page, key, randomNumber(0, 5))
        } else {
      ...
      for (let i = 0; i < randomNumber(1, 10); i++) {
        i !== 0 && await page.locator('[data-cy="add-content"]').click({ force: true })
